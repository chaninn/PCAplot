...
shinyUI(fluidPage(title="PCAplot: a web tool for generating PCA plot in R", theme=shinytheme("united"),
                  useShinyjs(),
                  navbarPage(strong("PCAplot"), collapsible = TRUE,
                             titleContent <- HTML("<b>PCAplot</b>: a web tool for generating PCA plot in R"),
                             tabPanel("Submit Job", titlePanel(titleContent),
                                      sidebarLayout(
                                        ...
                                        sidebarPanel(
                                          ...
                                          fileInput('file1', 'Upload Scores data',accept=c('.txt')),
                                          fileInput('file2', 'Upload Loadings data',accept=c('.txt')),
